...
export default function BarChart(props) {
  ...
  const [selectedBar, setSelectedBar] = React.useState(data[selectedBarIndex]);
  React.useEffect(() => {
  ...
  const Labels = args => {
    ...
    const labels = data.map((barData, index) => {
      ...
      return React.createElement(G, {
        ...
        textAnchor: 'middle'
      }, (value / total * 100).toFixed(1) + '%'), strokeLinecap !== 'butt' && React.createElement(Rect, {
        x: x(index),
  ...
  return React.createElement(View, {
    ...
    return React.createElement(View, {
      ...
      numberOfLines: 2,
      style: {
        color: selectedBar && selectedBar.key === chartData.key ? palette.gray_600 : palette.gray_500,
        textAlign: 'center',
        fontWeight: 'bold'
      }
